/*
 *  drivers/cpufreq/cpufreq_2phase_ondemand.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_2PHASE_ONDEMAND
static
#endif
struct cpufreq_governor cpufreq_gov_2phase_ondemand = {
	.name                   = "2phase_ondemand",
...
/* cpufreq_2phase_ondemand Governor Tunables */
 ...
 * rate is 10 ms because the user needs immediate reaction from 2phase_ondemand
...
skip_this_cpu:
		...
		for_each_online_cpu(cpu) {
			...
			if (dbs_info->cur_policy) {
				/* cpu using 2phase_ondemand, cancel dbs timer */
				mutex_lock(&dbs_info->timer_mutex);
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of 2phase_ondemand, waiting for disk IO is an
...
static void dbs_chown(void)
{
	...
	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/ignore_nice_load", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown ignore_nice_load error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/io_is_busy", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown io_is_busy error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/powersave_bias", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown powersave_bias error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/sampling_down_factor", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown sampling_down_factor error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/sampling_rate", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown sampling_rate error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/two_phase_freq", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown two_phase_freq error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/up_threshold", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown up_threshold error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/down_differential", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown down_differential error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/touch_poke", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown touch_poke error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/ui_sampling_rate", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown ui_sampling_rate error: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/2phase_ondemand/ui_counter", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_2phase_ondemand);
}

static void __exit cpufreq_gov_dbs_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_2phase_ondemand);
...
MODULE_DESCRIPTION("'cpufreq_2phase_ondemand' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_2PHASE_ONDEMAND
